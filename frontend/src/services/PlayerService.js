...
export async function getPlayersWithFiltersAsync(
  navigate,
  pageLength,
  pageNumber,
  isActive,
  ...
  try {
    const response = await API.get(
      `/player?pageSize=${pageLength}&pageNumber=${pageNumber}&isActive=${isActive}&name=${searchFilter}&orderBy=${orderBy}&sortOrder=${sortOrder}`,
      { headers: getHeaders() }
    );
    return response.data;
  } catch (error) {
